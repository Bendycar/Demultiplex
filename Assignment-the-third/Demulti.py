...
def reverse_complement(DNA: str) -> str:
    '''Simply takes a DNA strand, validates it, and returns the reverse complement if it is a valid DNA string. Case insensitive. Does not yet handle RNA'''
...
def open_files_for_writing(indices: set) -> tuple: #Tuple of two dictionaries, one for R1 and R2
    ...
    for index in indices:
        R1_files[index] = open(f"{output}/R1_{index}.fastq", "w")
...
def write_record(record1: list, record2: list, Index: str) -> None:
    ...
    if matched == True:
        ...
        for line in record2:
            fhr2.write(f"{line}\n")
    elif unknown == True or low_QScore == True: #I don't think the or statement is necessary, since every read with a low QScore is also marked as unknown.
        fhr1 = R1_files['Unknown'] #But this whole low QScore thing gave me a lot of trouble, and it works now, so I'm leaving it.
        ...
        for line in record2:
            fhr2.write(f"{line}\n")
    else: #If something for some reason doesn't fall into any of the buckets
        raise RuntimeError

def output_stats(matched_counts: dict, hopped_counts: dict, unknown_counts: dict) -> None:
...
def update_counts(index1: str, index2: str) -> None:
    ...
    if unknown == True:
        ...
        if (index1, index2) in matched_counts:
            ...
            matched_counts[index1, index2] = 1
    else: #Same thing as before, should never reach this statement
        raise RuntimeError
...
hopped_counts = {}
unknown_counts = {}
R1_files, R2_files = open_files_for_writing(index_set) #Dual assignment to the dictionaries generated by my open files function

with gzip.open(R1, "rt") as fh1, gzip.open(R2, "rt") as fh2, gzip.open(R3, "rt") as fh3, gzip.open(R4, "rt") as fh4:
    while True:
        matched, hopped, unknown, low_QScore = False, False, False, False #Fancy tuple packing from Leslie
        ...
        if not record1[0] or not record2[0] or not record3[0] or not record4[0]: #This line feels kinda sketchy, will test it thoroughly / ask Leslie
            break #Verdict: Fine because we can safely assume files are same size
        ...
        if index1 not in index_set or index2 not in RC_index_set: #First check if either index is unknown
            unknown = True
            RC_index2 = reverse_complement(index2) # type: ignore
        elif average_QScore(record2[3]) < QScore_cutoff or average_QScore(record3[3]) < QScore_cutoff: # type: ignore #Next check if indices are known but poor quality
            unknown = True #I should be able to do this just with the low_QScore variable, but I got confused and this works, so c'est la vie...
            low_QScore = True 
            RC_index2 = reverse_complement(index2) # type: ignore
            ...
            hopped = True
        elif RC_index_set[index2] == index1:
            RC_index2 = RC_index_set[index2] 
            matched = True
        else:
            raise RuntimeError
        ...
        append_indices(record1, index1, RC_index2) # type: ignore
        append_indices(record4, index1, RC_index2) # type: ignore

        update_counts(index1, RC_index2) # type: ignore
        write_record(record1, record4, index1) # type: ignore
        
        record1, record2, record3, record4 = [], [], [], []

close_all_files(R1_files, R2_files)

output_stats(matched_counts, hopped_counts, unknown_counts)
